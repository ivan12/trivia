...
export default function HostGame() {
      ...
      if (gameData.players) {
        ...
        Object.keys(gameData.players).forEach((playerId) => {
          playersList.push({
            ...
            answer: gameData.players[playerId].answer,
          })
        })
  ...
  useEffect(() => {
    ...
    const allAnswered = players.every((player) => player.answer && player.answer.questionIndex === currentQuestionIndex)
  ...
  const showResults = () => {
    ...
    const correctAnswers = players.filter(
      (player) => player.answer && player.answer.questionIndex === currentQuestionIndex && player.answer.isCorrect,
    )
  ...
  if (gamePhase === "results" && currentQuestionIndex >= 0 && currentQuestionIndex < questions.length) {
    const currentQuestion = questions[currentQuestionIndex]
            ...
            <CardContent className="p-6">
                ...
                {currentQuestion.options.map((option, index) => (
                  <div
                    ...
                    className={`h-24 ${colors[index]} p-4 flex items-center justify-center rounded-lg ${
                      index === currentQuestion.correctAnswer ? "ring-4 ring-green-300" : ""
                    }`}
                  >
                    <span className="text-2xl font-bold text-white text-center">
                      {option}
                      {index === currentQuestion.correctAnswer && <CheckCircle className="ml-2 h-6 w-6 inline" />}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="text-xl font-bold flex items-center mb-4">
                  ...
                  Correct Answer:{" "}
                  <span className="ml-2 text-green-600">{currentQuestion.options[currentQuestion.correctAnswer]}</span>
                </h3>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {players
                      ...
                      .sort((a, b) => (a.answer?.time || 0) - (b.answer?.time || 0))
                      .map((player, index) => {
                        const answerTime = player.answer?.time || 0

                        // Calculate points
                        const basePoints = 1000 * (currentQuestion.points || 1)
                        const speedFactor = Math.max(0, 1 - answerTime / 20000)
                        const points = Math.round(basePoints * speedFactor)
                        const bonusPoints = player.id === fastestCorrectPlayer ? 500 : 0
                            ...
                            <div className="flex items-center gap-3">
                              <div
                                className={`flex h-8 w-8 items-center justify-center rounded-full ${
                                  index === 0 ? "bg-yellow-400 text-white" : "bg-green-500 text-white"
                                } font-bold`}
                              >
                                {index + 1}
                              </div>
                              <div>
                                ...
                                <div className="text-xs text-gray-500">{(answerTime / 1000).toFixed(2)}s</div>
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="font-bold text-green-700">+{points + bonusPoints}</div>
                              {index === 0 && (
                                <div className="text-xs text-yellow-600 font-medium">Fastest! +500 bonus</div>
                              )}
                            </div>
                          </motion.div>
                        )
                  ...
                  {/* Incorrect answers */}
                  <h4 className="font-semibold text-gray-700 mt-4 mb-2">Incorrect Answers:</h4>
                  <div className="space-y-2">
                    {players
                      .filter(
                        (player) =>
                      ...
                      .map((player) => {
                        const answerOption =
                          player.answer?.option !== undefined
                            ? currentQuestion.options[player.answer.option]
                            : "No answer"
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center">
                              <span className="text-sm text-gray-500 mr-2">Answered: {answerOption}</span>
                  ...
                  {/* No answers */}
                  {players.filter((player) => !player.answer || player.answer.questionIndex !== currentQuestionIndex)
                    .length > 0 && (
                    <>
                      <h4 className="font-semibold text-gray-700 mt-4 mb-2">No Answer:</h4>
                      <div className="space-y-2">
                        {players
                          .filter((player) => !player.answer || player.answer.questionIndex !== currentQuestionIndex)
                          .map((player) => (
                            <motion.div
                              key={player.id}
                              initial={{ x: -20, opacity: 0 }}
                            ...
                            >
                              ...
                              <div className="flex items-center">
                                <span className="text-sm text-gray-500 mr-2">Time ran out</span>
  ...
  if (gamePhase === "question" && currentQuestionIndex >= 0 && currentQuestionIndex < questions.length) {
    const currentQuestion = questions[currentQuestionIndex]
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {currentQuestion.options.map((option, index) => (
              ...
              >
                <div className={`h-32 ${colors[index]} p-4 flex items-center justify-center`}>
          ...
          <div className="mt-4 rounded-lg bg-white p-4 shadow-lg">
            ...
            <div className="flex flex-wrap gap-2">
              {players.map((player, idx) => {
                const hasAnswered = player.answer && player.answer.questionIndex === currentQuestionIndex
                const bgColors = [
                ...
                return (
                  <div
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium border ${
                      hasAnswered ? "bg-purple-100 text-purple-800 border-purple-200" : bgColors[idx % bgColors.length]
                    } flex items-center`}
                  >
                    {player.name}

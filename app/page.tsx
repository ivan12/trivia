...
export default function KahootQuiz() {
  ...
  const joinGame = () => {
    ...
    router.push(`/player/join?gameId=${gameId.toUpperCase()}&playerName=${encodeURIComponent(playerName)}`)
  }
      ...
      setPlayers((prevPlayers) =>
        prevPlayers.map((player) => {
          ...
          if (playerAnswer) {
            ...
            const bonusPoints = playerAnswer.playerId === correctAnswers[0].playerId ? 500 : 0
        ...
        >
          <Card className="border-0 shadow-xl">
            <CardHeader className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-t-lg">
              <CardTitle className="text-center text-3xl font-bold text-white">Kahoot-Style Quiz</CardTitle>
            </CardHeader>
  ...
  if (showScoreboard && !gameOver) {
    ...
    const fastestPlayer = fastestCorrectPlayer ? players.find((p) => p.id === fastestCorrectPlayer) : null

    // Get the time for the fastest player
    const fastestTime = fastestPlayer && answers.find((a) => a.playerId === fastestPlayer.id)?.time
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-purple-700 to-indigo-900 p-4">
        <Card className="w-full max-w-md overflow-hidden">
          <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4 text-white">
            ...
            <p className="text-center text-purple-100">Question {currentQuestion + 1} completed</p>
          </div>
          ...
          <CardContent className="p-6">
            {fastestPlayer && (
              ...
              >
                ...
                <div className="text-2xl font-bold text-yellow-700">{fastestPlayer.name}</div>
                {fastestTime && (
                  <div className="text-sm font-medium text-yellow-600">{(fastestTime / 1000).toFixed(2)} seconds</div>
                )}
              </motion.div>
            )}
                      ...
                      <div className="text-right">
                        ...
                        {playerAnswer?.isCorrect && (
                          <div className="text-xs text-green-600">
                            +
                            {Math.round(1000 * (1 - (playerAnswer.time || 20000) / 20000)) +
            ...
            >
              <div
                className={`h-32 ${colors[index]} p-4 flex items-center justify-center ${
                  showAnswer && option === quizData[currentQuestion].correctAnswer ? "ring-4 ring-green-300" : ""
                }`}
              >
              ...
              {!showAnswer && (
                <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 p-2">
                  <div className="flex flex-wrap justify-center gap-2">
                    {players.map((player) => {
                      // Check if player already answered
                      const playerAnswered = answers.some((a) => a.playerId === player.id)
                      ...
                      return (
                        <Button
                          ...
                          className={`h-8 px-3 font-medium text-white rounded-full bg-gradient-to-r shadow-md transform transition-transform ${
                            playerColorClasses[playerColor % playerColorClasses.length]
                          } ${playerAnswered ? "opacity-50 cursor-not-allowed" : "hover:scale-105"}`}
                          onClick={() => !playerAnswered && recordAnswer(player.id, option)}
                          disabled={playerAnswered}
                        >
                          {presentationMode ? (playerAnswered ? "✓" : "...") : player.name}
                        </Button>
                      )
                    })}
                  </div>
        ...
        {showAnswer && (
          <div className="mt-6 rounded-lg bg-white p-4 shadow-lg">
            <h3 className="mb-4 text-xl font-bold flex items-center">
              ...
              Correct Answer: <span className="ml-2 text-green-600">{quizData[currentQuestion].correctAnswer}</span>
            </h3>

            <div className="space-y-3">
              <h4 className="font-semibold text-gray-700 mb-2">Correct Answers (Fastest First):</h4>
              {answers
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="font-medium">{player.name}</span>
                          <div className="text-xs text-gray-500">{(answer.time / 1000).toFixed(2)}s</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-green-700">+{totalPoints}</div>
                        {index === 0 && <div className="text-xs text-yellow-600 font-medium">Fastest! +500 bonus</div>}
                      </div>
                    </motion.div>
                  )
                })}
                      ...
                      >
                        ...
                        <div className="flex items-center">
                          <span className="text-sm text-gray-500 mr-2">
                            Answered: {quizData[currentQuestion].options.find((opt) => opt === answer.option)}
                          </span>
